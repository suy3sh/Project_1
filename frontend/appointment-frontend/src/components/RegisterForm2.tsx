        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-100 to-white px-4">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8">
                ...
                <form onSubmit={handleSubmit} className="mt-8 space-y-5">
                    ...
                    {/* Allergies Section */}
                    <div>
                        <p className="block text-sm mb-1 text-gray-600">Allergies (optional)</p>

                        {/* No Allergies Checkbox */}
                        <label className="flex items-center gap-2 text-gray-700 mb-2">
                            <input
                                type="checkbox"
                                checked={value.noAllergies || false}
                                onChange={() =>
                                    onChange({
                                        ...value,
                                        noAllergies: !value.noAllergies,
                                        allergyIds: !value.noAllergies ? [] : value.allergyIds,
                                    })
                                }
                            />
                            No Allergies
                        ...
                        {/* Allergy List */}
                        <div
                            className={`w-full rounded-lg border border-gray-300 px-4 py-2 max-h-40 overflow-auto space-y-2 pr-1 ${
                                value.noAllergies ? "opacity-50 pointer-events-none" : "focus:outline-none focus:ring-2 focus:ring-purple-400"
                            }`}
                        >
                            {allergies.map((a) => (
                                <label key={a.allergyId} className="flex items-center gap-2 text-gray-700">
